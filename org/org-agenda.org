...
:keymaps 'org-agenda-mode-map
  ...
  (kbd "C-x") #'org-clock-multi-agenda-clock-cancel-all
...
(defun kwrooijen/org-clock--open-clocked-today ()
  "Return minutes from currently running clock that fall within today."
  (when (org-clocking-p)
    (pcase-let* ((`(,start . ,end) (kwrooijen/org-clock--today-range))
                 (clock-start (float-time org-clock-start-time))
                 (now (float-time (current-time))))
      ;; overlap between [clock-start, now] and [start, end]
      (when (< clock-start end)
        (/ (max 0 (- (min now end)
                     (max clock-start start)))
           60)))))
