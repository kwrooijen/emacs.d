...
(defun kwrooijen/forge-test-file ()
  (interactive)
  (let ((file (replace-regexp-in-string (kwrooijen/project-root) "" (buffer-file-name))))
...
(defun kwrooijen/foundry-forge-script-broadcast ()
  ...
  (when (equal major-mode 'solidity-mode)
    (let* (;; (contract-name (kwrooijen/foundry-get-contract-name ))
              (default-directory (or (kwrooijen/project-root) default-directory))
              (file (replace-regexp-in-string (kwrooijen/project-root) "" (buffer-file-name)))
...
(defun kwrooijen/foundry-chisel-start ()
  ...
  (with-current-buffer "*chisel*"
    (setq default-directory (or (kwrooijen/project-root) default-directory))
