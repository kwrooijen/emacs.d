    ...
    (let* ((file (buffer-file-name))
      ...
      (when (and file id)
        (let ((key (cons file id)))
          ;; Show if clocked in OR paused
          (when (or (org-clock-multi--find-clock-by-key key)
...
(defun kwrooijen/org-todo-unarchive (&optional filter-tag)
  "Select a DONE task from agenda files and change it back to TODO.
When FILTER-TAG is non-nil, only show tasks that have that tag."
  (interactive)
  (let (entries max-tag-width max-closed-width)
        ...
        (org-with-wide-buffer
         ...
         (while (re-search-forward org-heading-regexp nil t)
           (when (and (member (org-get-todo-state) org-done-keywords)
                      (or (null filter-tag)
                          (member filter-tag (org-get-tags))))
    ...
    (if (null entries)
      ...
      (let* ((candidates
             ...
             (selection (completing-read "Unarchive: " collection nil t))
...
(defun kwrooijen/org-todo-unarchive-work ()
  ...
  (interactive)
  (kwrooijen/org-todo-unarchive "work"))
#+END_SRC
...
(kwrooijen/leader
  ...
  "au" '(kwrooijen/org-todo-unarchive-work :which-key "Unarchive"))
#+END_SRC
