             ...
             (while (re-search-forward org-heading-regexp nil t)
               (when (and (member (org-get-todo-state) states)
                 ...
                 (let* ((heading (org-get-heading t t t t))
                        ...
                        (closed (or (org-entry-get nil "CLOSED") ""))
                        (clocked (let ((id (org-entry-get nil "CLOCK_MULTI_ID")))
                                   (and id (org-clock-multi--find-clock-by-key id) t)))
                        (marker (point-marker)))
                   (push (list tag-str state closed heading marker clocked) entries)))))))
        (if (null entries)
          ...
          (let ((max-state-width (apply #'max (mapcar (lambda (e) (length (nth 1 e))) entries))))
            ...
            (let* ((candidates
                   ...
                   (group-fn (lambda (cand transform)
                               (if transform cand
                                 (if (get-text-property 0 'kwrooijen/clocked cand)
                                     "Clocked In"
                                   "Tasks"))))
                   (collection (lambda (str pred action)
                                 (if (eq action 'metadata)
