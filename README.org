...
(defun kwrooijen/subwindow-setup-hook-helm ()
  ;; Helm has special needs when it comes to getting the proper
  ;; buffer.
  (when (get-buffer (helm-buffer-get))
    (with-current-buffer (get-buffer (helm-buffer-get))
      (setq kwrooijen/subwindow-needs-style t))))

(defun kwrooijen/subwindow-restyle ()
  (dolist (win (window-list))
    (with-current-buffer (window-buffer win)
      (kwrooijen/subwindow-styling))))
#+END_SRC
...
(add-hook 'helm-after-initialize-hook #'kwrooijen/subwindow-setup-hook-helm)
...
Instantly go to the match after navigating to the candidate.
#+BEGIN_SRC emacs-lisp
