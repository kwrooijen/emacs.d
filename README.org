...
(kwrooijen/leader
  ...
  "c r" '(kwrooijen/rake :which-key "Rake"))
...
(kwrooijen/leader
  ...
  "f F" '(kwrooijen/find-file-at-point :which-key "Find File at Point")
...
(defun kwrooijen/rake--targets (rakefile)
  "Parse RAKEFILE and return a list of task names."
  (when (file-exists-p rakefile)
    (let (targets)
      (with-temp-buffer
        (call-process "rake" nil t nil "-f" rakefile "-T")
        (goto-char (point-min))
        (while (re-search-forward "^rake \\([^ ]+\\)" nil t)
          (push (match-string 1) targets)))
      (nreverse targets))))

(defun kwrooijen/rake ()
  "Select a Rakefile task via completing-read and run it in a unique buffer."
  (interactive)
  (let* ((default-directory (or (kwrooijen/project-root) default-directory))
         (rakefile (expand-file-name "Rakefile" default-directory))
         (targets (kwrooijen/rake--targets rakefile))
         (target (completing-read "Rake: " targets nil t))
         (buf-name (format "rake:%s:%s" (kwrooijen/project-name) target)))
    (pop-to-buffer (make-comint-in-buffer buf-name nil "rake" nil target))))

(defmacro comment (&rest _))
#+END_SRC

#+BEGIN_SRC emacs-lisp
(defun kwrooijen/find-file-at-point ()
  "Open the file path at point if it exists."
  (interactive)
  (let ((path (thing-at-point 'filename t)))
    (if (and path (file-exists-p (expand-file-name path)))
...
#+BEGIN_SRC emacs-lisp
(defun bw-unlock ()
  "Unlock Bitwarden and set BW_SESSION in Emacs process environment."
  (interactive)
  (let* ((password (read-passwd "Bitwarden Master Password: "))
         (session (string-trim
                   (with-temp-buffer
                     (let ((process-environment
                            (cons (concat "BW_MASTER_PASS=" password)
                                  process-environment)))
                       (call-process "bw" nil t nil
                                     "unlock" "--raw"
                                     "--passwordenv" "BW_MASTER_PASS"))
                     (buffer-string)))))
    (setenv "BW_SESSION" session)
    (message "BW_SESSION set")))
...
:STRAIGHT: (agent-shell-manager :host github :repo "kwrooijen/agent-shell-manager" :branch "feature/status-change-hook")
:END:
...
(kwrooijen/leader
 ...
 "f l" 'kwrooijen/agent-shell-switch-agent
 "a l" 'kwrooijen/agent-shell-switch-agent)
#+END_SRC
...
(defvar kwrooijen/org-todo-find-map
  (let ((map (make-sparse-keymap)))
    ...
    (define-key map (kbd "C-l") #'kwrooijen/org-todo-find--open-shell)
             ...
             (while (re-search-forward org-heading-regexp nil t)
               (when (and (member (org-get-todo-state) states)
                 ...
                 (let* ((heading (org-get-heading t t t t))
                        ...
                        (agent-status
                         (if (and project branch)
                             (let* ((wt-path (expand-file-name
                                              (org-agent-shell--derive-worktree-name project branch)
        ...
        (if (null entries)
          ...
          (setq entries (sort entries (lambda (a b)
                                        (let ((ca (nth 5 a)) (cb (nth 5 b))
                                              ...
                                              (sa (nth 1 a)) (sb (nth 1 b))
                                              (state-order '("IN PROGRESS" "TODO" "WAITING")))
                                          (cond
                                           ((and ca (not cb)) t)
                                           ((and (not ca) cb) nil)
                                           ((and pa (not pb)) t)
                                           ((and (not pa) pb) nil)
                                           ((/= (cl-position sa state-order :test #'equal)
                                                (cl-position sb state-order :test #'equal))
                                            (< (cl-position sa state-order :test #'equal)
                                               (cl-position sb state-order :test #'equal)))
