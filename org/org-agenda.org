...
(defun kwrooijen/org-todo-find--preview ()
  ...
  (when-let* ((cand (nth vertico--index vertico--candidates))
              (marker (cdr (assoc cand kwrooijen/org-todo-find--candidates))))
    (let* ((old (get-buffer "*org-todo-preview*"))
           (popup-win (and old (get-buffer-window old)))
           ;; Create the edit-indirect buffer without displaying it
           (buf (save-window-excursion
                  (with-current-buffer (marker-buffer marker)
                    ...
                    (current-buffer)))))
      (cond
       ;; Same buffer and already visible — do nothing
       ((and (eq buf old) popup-win (window-live-p popup-win)))
       ;; Same buffer but not visible — re-display it
       ((eq buf old)
        (popwin:display-buffer buf))
       ;; Different buffer — swap it in
       (t
        (when old (kill-buffer old))
        (with-current-buffer buf
          (rename-buffer "*org-todo-preview*"))
        (if (and popup-win (window-live-p popup-win))
            (set-window-buffer popup-win buf)
          (popwin:display-buffer buf)))))
             ...
             (while (re-search-forward org-heading-regexp nil t)
               (when (and (member (org-get-todo-state) states)
                 ...
                 (let* ((heading (org-get-heading t t t t))
                        ...
                        (marker (point-marker)))
                   (push (list tag-str state closed heading marker clocked paused) entries)))))))
        (if (null entries)
