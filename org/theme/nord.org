...
(defun kwrooijen/subwindow-setup-after-display (&rest _)
  (when-let ((win (or (helm-window) (get-buffer-window (current-buffer)))))
...
(defun kwrooijen/subwindow-setup-hook ()
  (run-at-time
   0 nil
   #'kwrooijen/subwindow-setup-after-display
   (current-buffer)))


(add-hook 'helm-after-initialize-hook #'kwrooijen/subwindow-setup-hook)
(add-hook 'magit-mode-hook #'kwrooijen/subwindow-setup-hook)
(add-hook 'eshell-mode-hook #'kwrooijen/subwindow-setup-hook)
(add-hook 'vterm-mode-hook #'kwrooijen/subwindow-setup-hook)
